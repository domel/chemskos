...
if (sizeof($parts) <= 2) {
  ...
  if (array_key_exists($parts[1], $config->getLanguages())) { // global pages
        ...
        ($parts[2] == 'about' || $parts[2] == 'feedback' || $parts[2] == 'structure' || $parts[2] == 'sparql' || $parts[2] == 'about-us' || $parts[2] == 'terms' || $parts[2] == 'contact' || $parts[2] == 'contact' || $parts[2] == 'search') ? $request->setPage($parts[2]) : $request->setPage('');
        if ($request->getPage() == '') {
            $controller->invokeVocabularies($request);
        } elseif ($request->getPage() == 'about-us') {
            $controller->invokeFaboutPage($request);
        } elseif ($request->getPage() == 'terms') {
            $controller->invokeFtermsPage($request);
        } elseif ($request->getPage() == 'contact') {
            $controller->invokeFcontactPage($request);
        } elseif ($request->getPage() == 'publication') {
            $controller->invokeFpublicationPage($request);
        } elseif ($request->getPage() == 'structure') {
            $controller->invokeStructurePage($request);
        } elseif ($request->getPage() == 'sparql') {
            $controller->invokeSparqlPage($request);
